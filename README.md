...
Четвертый параметр - булевая величина, определяет, искать ли в начале, поставщика работы с базой данных\
...
```
При работе с Sqlite, обращайте внимание на изоляцию транзакций, из разных потоков,\
из за специфики файловой стрктуры базы.
...
тип должен иметь открытый коструктор.
```C#
  /// <summary>
...
Ошибки не будет, все объекты для доступа к базе получат Dispose.\
Но будет вызван финализатор для сессии, когда подойдет время для очистки кучи.\
Если вы используете тразакцию, лучше все же использовать using.(try/finaly)\
